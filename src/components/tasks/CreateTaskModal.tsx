  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (activePlatforms.length > 0) setCurrentPlatformId(activePlatforms[0].id)
      } finally {
  ...
  return (
    <Modal open={open} onClose={onClose} title={
      <div className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.2em]">
        <span className="opacity-40">Marketing Canvas</span>
        <span className="opacity-20">/</span>
        <span>Establish New Task</span>
      </div>
    } wide noHeaderStyles>
      <form onSubmit={submit} className="bg-[#0B0D13]/95 text-white p-8 space-y-12 animate-in fade-in duration-500 rounded-[2.5rem] relative z-[50]">
        ...
        <div className="grid gap-x-12 gap-y-8 lg:grid-cols-2">
          ...
          <div className="space-y-8">
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              ...
              />
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#4F5B76]">Deadline</label>
                <div 
                ...
                >
                  <Input 
                    ...
                    className="h-12 bg-black/40 border-[#1E2330] font-black text-[11px] uppercase pr-10 [color-scheme:dark]"
                  />
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between border-b border-white/5 pb-4">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-sm font-black uppercase tracking-[0.2em]">Subtask Architecture</h3>
            </div>
            <span className="rounded-lg bg-white/5 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-[#4F5B76]">
              {subtaskDrafts.length} Units Pending
            </span>
          </div>
            ...
            <div className="flex items-center gap-4 rounded-2xl border border-[#1E2330] bg-[#161B26]/50 p-2 pl-6 focus-within:border-[var(--color-accent)]/40 transition-all">
              ...
              <div className="flex items-center gap-2 pr-2">
                <div className="relative tactical-dropdown">
                  <button
                    type="button"
                    onClick={() => { setPlatformMenuOpen(!platformMenuOpen); setAssigneeMenuOpen(false); }}
                    className={`h-10 min-w-[140px] rounded-xl bg-black/60 border border-white/5 px-4 text-[10px] font-black uppercase text-white/70 hover:text-white hover:border-white/10 transition-all flex items-center justify-between gap-2 cursor-pointer ${platformLoading ? 'animate-pulse opacity-50' : ''}`}
                    disabled={platformLoading}
                  >
                    <span className="truncate">
                      {platforms.find(p => p.id === currentPlatformId) 
                        ? (PLATFORM_LABEL[platforms.find(p => p.id === currentPlatformId)!.platform as keyof typeof PLATFORM_LABEL] || 'UNIT') 
                        : (platforms.length > 0 ? 'Select Platform' : 'No Platforms')}
                    </span>
                  ...
                  {platformMenuOpen && (
                    <div className="absolute top-full right-0 mt-2 z-[150] w-48 overflow-hidden rounded-2xl border border-white/10 bg-[#161B26] p-2 shadow-2xl animate-in fade-in slide-in-from-top-2">
                      <div className="max-h-48 overflow-y-auto scrollbar-hide">
                        {platforms.length === 0 && (
                          <div className="p-3 text-[9px] font-black uppercase text-[#4F5B76] text-center">Operational Void</div>
                        )}
                        {platforms.map(p => (
                          <button
                            key={p.id}
                            ...
                            onClick={() => { setCurrentPlatformId(p.id); setPlatformMenuOpen(false); }}
                          >
                            {PLATFORM_LABEL[p.platform as keyof typeof PLATFORM_LABEL] || p.platform.toUpperCase()}
                          </button>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex items-center gap-3">
                        <span className="rounded-md bg-[#EE4667]/10 px-2 py-0.5 text-[9px] font-black uppercase tracking-widest text-[#EE4667]">
                          {platform ? PLATFORM_LABEL[platform.platform as keyof typeof PLATFORM_LABEL] : 'Creative'}
                        </span>
                        <span className="text-[9px] font-black uppercase tracking-widest text-[#4F5B76] flex items-center gap-1.5">
                          <Calendar className="h-3 w-3" /> ASAP
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex items-center justify-between pt-12 border-t border-white/5">
          <div className="flex items-center gap-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-[#4F5B76]">Collaborators Assigned</span>
          </div>
